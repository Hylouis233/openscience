...
2. 本插件的 citation-verify、claim-check 可用；science-literature 插件的
   literature-search 可用（补检依赖它；若不可用，闭环退化为「只查不补」，
   并在汇总中明确说明）。
...
- 逐条 query 调用 science-literature 的 literature-search；provider 选择遵循
