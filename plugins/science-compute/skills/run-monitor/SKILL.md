...
```bash
python <插件路径>/skills/run-monitor/scripts/run_task.py \
...
```bash
python <插件路径>/.../run_task.py status --name cycle-fit   # 单个 run 的状态
python <插件路径>/.../run_task.py list                       # 全部 run 一览
python <插件路径>/.../run_task.py tail --name cycle-fit --lines 20
```
...
# 2. 交还控制权；过一阵子在自然节点查询
python <插件路径>/skills/run-monitor/scripts/run_task.py status --name cycle-fit
#    → status: running（继续干别的，不写 sleep 循环）

# 3. 再次查询：status: exited, exit_code: 0
python <插件路径>/skills/run-monitor/scripts/run_task.py tail --name cycle-fit --lines 10

# 4. 产物归位 + provenance 登记（record_run 的 note 写 run_id 与 cmd）
python <science-core 插件路径>/skills/provenance-record/scripts/record_run.py \
