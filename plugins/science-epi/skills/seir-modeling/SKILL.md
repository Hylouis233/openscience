...
二乘/MCMC）时转 science-compute，本技能不拟合。
...
- **不做参数拟合**：不用观测数据估计参数（最小二乘/极大似然/MCMC
  交给 science-compute 或专业环境）；本技能只做前向推演。
...
3. 建议下一步：
   - 需要拟合真实数据或更复杂模型 → science-compute；
   - 结果写入报告 → `epi-writing`（附参数三件套与情景表）；
   - 参数来源需补文献 → science-literature 的 literature-search。
