...
铁律（与 science-core guardrails 一致，此处为域内具体化）：

1. 凡写入产物的数据事实必须带来源标签；标签描述出处，不暗示置信度。
2. 数据文件落地即冻结：进 `data/` 后不改名、不编辑、不覆盖（原始数据只读
   原则，见 science-core 的 research-workspace）。
...
4. 确认工作区已初始化（`data/` 目录存在）；未初始化时引导用户先运行
   science-core 的 research-workspace。
...
4. **登记 provenance**：用 science-core 的 provenance-record 登记，note 中
