...
| literature | 摸清该问题的研究现状 | science-literature 插件：literature-search、literature-survey | 文献库 + 综述稿 |
...
| writing | 写成可交付的文稿 | review-writing + science-verify 插件：citation-verify | 论文/报告草稿 |
...
2. 调用 science-literature 插件的 `literature-search` 按画像中的数据源偏好检索（画像写"无权限"的库不碰）；
...
**执行模式**：计算能力由 science-compute 插件提供，已交付 5 个 skill（python-analysis / r-analysis / remote-compute / hpc-slurm / run-monitor）。本阶段的做法：
...
**计算能力**：分析计算优先调用 science-compute 插件——本地分析用 python-analysis / r-analysis，远程与长任务用 remote-compute / hpc-slurm / run-monitor。
...
3. 参考文献条目生成后，调用 science-verify 插件的 `citation-verify` 逐条核验（DOI、作者、年份、期刊），核验未通过的条目不得进入终稿；
4. 论文级结论定型时，按 `evidence-capsule` 冻结支撑该结论的产物集合；
5. 草稿完成后调用 `agents/reviewer.md` 做发布前审查（guardrail 第 7 条），error 清零后才算本阶段完成；
...
- 不亲自执行远程任务提交、批量下载等计算动作：这些委托给 science-compute 插件（remote-compute / hpc-slurm / run-monitor），本技能只做调度与状态管理。
