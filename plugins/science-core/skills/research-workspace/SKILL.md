...
├── reviews/         # 收到的审稿意见、reviewer agent 的审查记录
├── scripts/         # 可重复执行的脚本（分析、作图、数据处理）
├── output/          # 各 skill 的运行产物（结构见"产物路径契约"）
└── .openscience/    # 工作台元数据：provenance.jsonl、env/、watch-seen.json 等
```
...
- **.openscience/**：隐藏目录，记录工作台的运行状态。用户不需要直接编辑，但可以随时查看。`provenance-record` 与 `literature-watcher` 写这里。
...
6. **已登记的例外**：science-verify 的 evidence-loop 使用轮次目录 `output/evidence-loop/<slug>/<round>/`，是本契约的显式例外，替代 `latest/`（见该 skill 的产物结构说明）。
