...
科研工作台里有多处需要"审查"：reviewer agent 审论文草稿、citation-verify 核验引用、stage-gate 汇总风险。如果每家输出一种格式，下游（用户、stage-gate、其他 skill）就要学 N 种方言。本契约规定**唯一**的结构化审查输出格式：所有审查方说同一种话，所有消费方只用一种解析。

本技能是库技能：不被用户直接调用，由核验类 skill 与 reviewer agent 在各自正文中引用本文件，保证输出一致。
...
- **reviewer agent**：完整遵循本契约；审查清单见 agents/reviewer.md。
...
- 不定义审查清单本身：审什么由各审查方自行规定（reviewer agent 有自己的清单，citation-verify 有自己的核验项），本契约只规定"审完怎么说"。
